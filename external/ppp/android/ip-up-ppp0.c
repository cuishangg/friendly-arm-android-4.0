...
static char propstr1[256];	// hold "net.$LINKNAME.xxx"
...
__android_log_print(ANDROID_LOG_INFO, "ZTE-ip-up-ppp0", "before memset \n");
    /* Clean out the RTREQ structure */
    memset(&rt, 0, sizeof(rt));
__android_log_print(ANDROID_LOG_INFO, "ZTE-ip-up-ppp0", "after memset \n");
...
__android_log_print(ANDROID_LOG_INFO, "ZTE-ip-up-ppp0", "before set iplocal \n");
   ...
   // set net.$LINKNAME.gw prop to $DNS1
	strcpy(propstr1, propstr);
	strcat(propstr1, "gw");
	char *gw = getenv("DNS1");
	property_set(propstr1, gw ? gw : "");
	// Fixme:  "net.gw" should be setup in framework code instead of in ip-up-ppp0 script
	property_set("net.gw", gw ? gw : "");
        __android_log_print(ANDROID_LOG_INFO, "ip-up-ppp0", "Setprop %s to %s", propstr1, gw ? gw : "");
        __android_log_print(ANDROID_LOG_INFO, "ip-up-ppp0", "Setprop %s to %s", "net.gw", gw ? gw : "");
    }
