...
int FimgApiStretch(Fimg *fimg, const char *func_name)
{
    ...
    if (!(fimg->matrixSx == 1.0 && fimg->matrixSy == 1.0) &&
        fimg->srcAddr != NULL && (fimg->srcW != fimg->dstW || fimg->srcH != fimg->dstH)) {
        scale.mode = SCALING_MODE;
        ...
        if (fimg->matrixSx == 0 && fimg->matrixSy == 0) {
                ...
                scale.dst_w = (int)(fimg->srcW * fimg->matrixSx);
                scale.dst_h = (int)(fimg->srcH * fimg->matrixSy);
        }
        cmd.scaling = &scale;
    } else if (!(fimg->matrixSx == 1.0 && fimg->matrixSy == 1.0) &&
               fimg->mskAddr != NULL && (fimg->mskW != fimg->dstW || fimg->mskH != fimg->dstH)) {
        scale.mode = SCALING_MODE;
        scale.factor = SCALING_PIXELS;
        ...
        if (fimg->matrixSx == 0 && fimg->matrixSy == 0) {
            ...
            scale.dst_w = (int)(fimg->mskW * fimg->matrixSx);
            scale.dst_h = (int)(fimg->mskH * fimg->matrixSy);
        }
        cmd.scaling = &scale;
    } else
        cmd.scaling = NULL;
