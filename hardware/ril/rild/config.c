...
{   
    ...
    for (i = 0; i < strlen(line); i++) { 
        if (isgraph(line[i])) {            /* 检查第一个可打印字符，是#就认为该行是注释 */
            if ( line[i] == '#' ) {
                return 1;   
            } else {
                return 0;
...
{   
    ...
    for (start = 0; start < strlen(line); start++) {   
        if (isgraph(line[start])) {   
            if ( line[start] != '[' ) {       /* 第一个可打印字符不是[就退出函数 */
                return 0;   
            } else {
                break;
            }
    ...
    for (end = strlen(line); end > 1; end--) {   
        if (isgraph(line[end])) {                 /* 最后一个可打印字符不是]就退出 */
            if (line[end] != ']') {
                return 0;
            } else {
                break;
    ...
    if (end - start < 2) {   /* [和]之间没有字符就退出 */
        return 0;
    }

    for (mid = start + 1; mid < end; mid++) {
        if (!isalnum(line[mid])) {            /* 不是英文和数字就退出函数 */
            return 0;
        }
    }
...
{   
    ...
    for (start = 0; start < strlen(line); start++) {   
        if ( isgraph(line[start]) ) {   
            if ( !isalnum(line[start]) ) {             /* 第一个可打印字符不是英文和数字就退出 */
                return 0;   
            } else {
                break;
            }
    ...
    for (end = start; line[end] != '=' && end < strlen(line); end++) {
        if ( !isalnum(line[end]) && line[end] != ' ' && line[end] != '\t' ) {     /* 不是英文和数字以及空格和Tab字符就退出函数 */
            return 0;
        }
    }   
...
{   
    ...
    for (start = 0; line[start] != '=' && start < strlen(line); start++) {   /* 找到=的索引 */
    }   

    for (start = start + 1; !isgraph(line[start]) && start < strlen(line); start++) {  /* 定位于=号后面第一个可打印字符 */
    }   
