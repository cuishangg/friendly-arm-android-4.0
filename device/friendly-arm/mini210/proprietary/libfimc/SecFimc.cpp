...
bool SecFimc::create(int fimc_dev, int fimc_mode, unsigned int buf_num)
{
    if(mFlagCreate == true)
    {
    ...
    LOGD("SecFimc::create(fimc_dev: %d, fimc_mode: %d, buf_num: %d)\n", fimc_dev, fimc_mode, buf_num);

    /*
    if(fimc_mode == FIMC_OVLY_DMA_AUTO)
        LOGD("############# SecFimc::create(FIMC_OVLY_DMA_AUTO) called \n");
    else if(fimc_mode == FIMC_OVLY_NONE_SINGLE_BUF)
        LOGD("############# SecFimc::create(FIMC_OVLY_NONE_SINGLE_BUF) called \n");
    else
        LOGD("############# SecFimc::create(%x) called \n", fimc_mode);
    */
    char node[20];
    struct v4l2_format fmt;
    struct v4l2_control     vc;
    ...
    if(mS5pFimc.dev_fd == 0) {
        ...
        if(mS5pFimc.dev_fd < 0) {
            ...
            LOGD("fimc%d fd = %d", fimc_dev, (int)mS5pFimc.dev_fd);
    }
#ifdef DEBUG_LIB_FIMC
    ...
    if (!(mFimcCap.capabilities & V4L2_CAP_STREAMING)) {
        LOGE("%s::%s has no streaming support\n", __func__, node);
        return false;
    }

    if (!(mFimcCap.capabilities & V4L2_CAP_VIDEO_OUTPUT)) {
        LOGE("%s::%s is no video output\n", __func__, node);
        return false;
    }
...
#ifdef DEBUG_LIB_FIMC
    LOGD("%s:: Fimc reserved memory =0x%8x\n", __func__, mFimcRrvedPhysMemAddr);
#endif
    ...
    if (ret < 0) {
        LOGE("Error in video VIDIOC_S_CTRL - V4L2_CID_OVLY_MODE (%d)\n",ret);
        mFimcOvlyMode = FIMC_OVLY_NOT_FIXED;
    }
...
bool SecFimc::destroy()
{
    ...
    LOGD("SecFimc::destroy()\n");

    /*
    if(mFimcOvlyMode == FIMC_OVLY_DMA_AUTO)
        LOGD("############# SecFimc::destroy(FIMC_OVLY_DMA_AUTO) called \n");
    else if(mFimcOvlyMode == FIMC_OVLY_NONE_SINGLE_BUF)
        LOGD("############# SecFimc::destroy(FIMC_OVLY_NONE_SINGLE_BUF) called \n");
    else
        LOGD("############# SecFimc::destroy(%x) called \n", mFimcOvlyMode);
    */
#ifdef DEBUG_LIB_FIMC
    LOGD("%s", __func__);
#endif
...
{
#ifdef DEBUG_LIB_FIMC
    LOGD("%s:: mFimcRrvedPhysMemAddr = 0x%8x",
            __func__, mFimcRrvedPhysMemAddr);
#endif
    if(mFlagCreate == false)
...
{
#ifdef DEBUG_LIB_FIMC
    LOGD("%s:: fimc version = 0x%x", __func__, mS5pFimc.hw_ver);
#endif
    if(mFlagCreate == false)
        LOGE("%s:: libFimc is not created", __func__);
...
#ifdef DEBUG_LIB_FIMC
    LOGD("%s:: format= %d, width = %, height= %d, cropW= %d, cropH= %d", __func__, colorFormat, width, height, *cropWidth, *cropHeight);
#endif
                           ...
                           int* colorFormat)
{
    ...
    *colorFormat = fmt.fmt.pix.pixelformat;
    ...
    if (ret < 0)
    {
        LOGE("%s:: Error in video VIDIOC_G_CROP\n", __func__);
        return false;
    }
                           ...
                           int* colorFormat)
{
    ...
    *colorFormat = fbuf.fmt.pixelformat;
    ...
    if (ret < 0)
    {
        LOGE("Error in video VIDIOC_G_FMT (%d)\n", ret);
        return false;
    }
...
{
#ifdef DEBUG_LIB_FIMC
    LOGD("%s :: physYAddr = 0x%8x, physCbAddr = 0x%8x",
...
bool SecFimc::setDstPhyAddr(unsigned int physYAddr,
                            unsigned int physCbAddr,
                            unsigned int physCrAddr)
{
    struct v4l2_framebuffer fbuf;
...
{
#ifdef DEBUG_LIB_FIMC
    LOGD("%s :: rotVal = %d", __func__, rotVal);
#endif
    struct v4l2_control vc;
    int ret_val;
    ...
    {
        LOGE("[%s] Fimc stream on", __func__);
        return false;
    }
...
{
#ifdef DEBUG_LIB_FIMC
    LOGD("%s :: enable= %d, alpha = %d", __func__, (int)enable, alpha);
#endif
    int ret;
    struct v4l2_framebuffer fbuf;
    struct v4l2_format fmt;
...
{
#ifdef DEBUG_LIB_FIMC
    LOGD("%s :: enable= %d", __func__, (int)enable);
#endif
    if(mFlagCreate == false)
    {
        LOGE("[%s] libFimc is not created", __func__);
...
{
#ifdef DEBUG_LIB_FIMC
    LOGD("%s :: enable= %d, colorKey = 0x%8x", __func__, (int)enable, colorKey);
#endif
    int ret;
    struct v4l2_framebuffer fbuf;
    struct v4l2_format fmt;
...
bool SecFimc::streamOn(void)
{
    ...
    {
        LOGE("%s::fimc_v4l2_stream_on(V4L2_BUF_TYPE_VIDEO_OUTPUT) fail\n", __func__);
        return false;
    }
...
bool SecFimc::streamOff(void)
{
    ...
    {
        LOGE("%s::fimc_v4l2_stream_off() fail\n", __func__);
        return false;
    }
...
bool SecFimc::queryBuffer(int index, struct v4l2_buffer *buf)
{
    ...
    if(index >= mBufNum)
    {
        LOGE("[%s] queryBuffer index is invalid (%d)", __func__, index);
    }
...
bool SecFimc::queueBuffer(int index)
{
    ...
    if(index >= mBufNum)
    {
        LOGE("[%s] queueBuffer index is invalid (index : %d)(mBufNum : %d)",
    ...
    {
        LOGE("%s :: source params are not set", __func__);
        return false;
    }
    if(mFlagSetDstParam == false)
    {
        LOGE("%s :: destination params are not set", __func__);
        return false;
    }
    ...
    {
        LOGE("%s :: fimc_v4l2_queue(index : %d) (mBufNum : %d) fail",
            __func__, index, mBufNum);
        return false;
    }
    return true;
...
bool SecFimc::handleOneShot()
{
    ...
    if(fimc_v4l2_stream_on(mS5pFimc.dev_fd, V4L2_BUF_TYPE_VIDEO_OUTPUT)< 0)
    {
        LOGE("%s :: fimc_v4l2_stream_on(V4L2_BUF_TYPE_VIDEO_OUTPUTd) fail", __func__);
        return false;
    }

    if(fimc_v4l2_queue(mS5pFimc.dev_fd, &fimc_src_buf, 0) < 0)
    {
        LOGE("%s :: fimc_v4l2_queue(index : %d) (mBufNum : %d) fail", __func__, 0, mBufNum);
        return false;
    }

    if(fimc_v4l2_dequeue(mS5pFimc.dev_fd) < 0)
    {
        LOGE("%s :: fimc_v4l2_dequeue (mBufNum : %d) fail", __func__, mBufNum);
        return false;
    }

    if(fimc_v4l2_stream_off(mS5pFimc.dev_fd) < 0)
    {
        LOGE("%s :: fimc_v4l2_stream_off() fail", __func__);
        return false;
    }
