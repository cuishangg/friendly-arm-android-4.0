...
MemoryHeapBaseIon::MemoryHeapBaseIon(size_t size, uint32_t flags, char const *name)
{
    ...
    if (mIonClient < 0) {
        mIonClient = -1;
        LOGE("MemoryHeapBaseIon : ION client creation failed");
    }
    void* base = NULL;
    int fd = ion_alloc(mIonClient, size, 0, ION_HEAP_EXYNOS_CONTIG_MASK);

    LOGE_IF(fd < 0, "MemoryHeapBaseIon : ION memory allocation failed");
    if (fd < 0) {
        LOGE("MemoryHeapBaseIon : ION memory allocation failed");
    } else {
        flags |= USE_ION_FD;
        ...
        if (base != MAP_FAILED)
            ...
            LOGE("MemoryHeapBaseIon : mmap failed");
    }
}

MemoryHeapBaseIon::MemoryHeapBaseIon(int fd, size_t size, uint32_t flags, uint32_t offset)
{
    LOGE_IF(fd < 0, "MemoryHeapBaseIon : file discriptor error. fd is not for ION Memory");
    mIonClient = ion_client_create();
    if (mIonClient < 0) {
        mIonClient = -1;
        LOGE("MemoryHeapBaseIon : ION client creation failed");
