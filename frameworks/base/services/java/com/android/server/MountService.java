...
import com.android.server.UDiskDSocket;
...
class MountService extends IMountService.Stub
    ...
    private UDiskDSocket udiskdSocket = null;
    ...
    public MountService(Context context) {
        ...
        udiskdSocket = new UDiskDSocket( new UDiskDSocket.RecvCallBack() {
            public void responseIncoming(String response)
            {
            processUDiskDResponse(response);
            }
            });
    }

    private void displayUDiskStatusIcon() {
        Intent notifyIntent=new Intent();
        notifyIntent.setComponent(new ComponentName("com.friendlyarm.usbdisk", "com.friendlyarm.usbdisk.MainActivity"));
        notifyIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        PendingIntent appIntent = PendingIntent.getActivity(mContext, 0,
            notifyIntent, 0);
        Notification myNoti = new Notification();
        myNoti.icon = com.android.internal.R.drawable.usbdisk;
        myNoti.tickerText = "USB mass storage has been mounted.";
        myNoti.defaults = Notification.DEFAULT_LIGHTS;
        myNoti.setLatestEventInfo(mContext, "USB Mass Storage", "USB mass storage has been mounted.", appIntent);
        myNotiManager.notify(0, myNoti);
    }

    private void processUDiskDResponse(String response)
    {
        if (response.startsWith("@ERROR")) {
            Slog.e(TAG, "#### Error: Can't get response from udiskd socket!");
            return ;
        } else if (response.startsWith("RESPONSE CONNECT")) {
            udiskdSocket.sendRequest("REQUEST UDISKSTATUS");
            udiskdSocket.recvResponse();
        } else if (response.startsWith("RESPONSE UDISKSTATUS MOUNTED")) {
            displayUDiskStatusIcon();
        } else if (response.startsWith("RESPONSE UDISKSTATUS UMOUNTED")) {
            removeUDiskStatusIcon();
        } else if (response.startsWith("RESPONSE UMOUNT OK")) {
        } else if (response.startsWith("NOTIFY USBSTATUS MOUNTED")) {
            displayUDiskStatusIcon();
        } else if (response.startsWith("NOTIFY USBSTATUS UMOUNTED")) {
            removeUDiskStatusIcon();
        }
    }

    private void removeUDiskStatusIcon() {
