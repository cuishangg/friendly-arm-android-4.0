...
if [ "${PRODUCT}" = "mini210" ]; then
    ...
    echo -n "Install google apps ..."
    [ -f "vendor/friendly-arm/mini210/gapps-ics-20120429.tgz" ] && \
        tar xzf "vendor/friendly-arm/mini210/gapps-ics-20120429.tgz" -C rootfs_dir
    echo "...done"
    ...
    if [ "$1" == "image" ]; then
        ...
        mkyaffs2image-mlc2 rootfs_dir rootfs_android-mlc2.img
        md5sum rootfs_android.img
        md5sum rootfs_android-mlc2.img
    fi
fi
...
SpeechRecorder.apk 
AndroidMarket.apk          
droidrecord_2.11.apk  
ES_TaskManager.apk
pandaspace_web_v3.0.8.apk
iReader.apk
BluetoothFileTransfer.apk
}
